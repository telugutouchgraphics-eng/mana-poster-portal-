...
function photoEdgeStyle(config: PosterPersonalization): React.CSSProperties {
  if (config.edgeStyle === "sharp") {
    return {};
  }
  return {
    WebkitMaskImage:
      "linear-gradient(to bottom, rgba(255,255,255,1) 0%, rgba(255,255,255,1) 72%, rgba(255,255,255,0.95) 84%, rgba(255,255,255,0) 100%)",
    maskImage:
      "linear-gradient(to bottom, rgba(255,255,255,1) 0%, rgba(255,255,255,1) 72%, rgba(255,255,255,0.95) 84%, rgba(255,255,255,0) 100%)",
  };
}
        ...
        {loading ? (
          ...
          rows.map((row) => {
            ...
            return (
              <article
                key={row.id}
                className="grid gap-5 rounded-[26px] border border-[var(--portal-border)] bg-white p-5 shadow-[0_10px_26px_rgba(15,23,42,0.04)] lg:grid-cols-[400px_minmax(0,1fr)]"
              >
                <div className="rounded-[22px] border border-[var(--portal-border)] bg-[var(--portal-surface-soft)] p-2.5">
                    ...
                    <div className="relative overflow-hidden bg-black">
                      ...
                      >
                        ...
                        <img
                          ...
                          alt="Creator sample"
                          className={`h-full w-full object-contain object-top ${photoShapeClass(config.photoShape)}`}
                          style={{
                            ...photoEdgeStyle(config),
                            objectPosition:
                              config.photoRenderMode === "cutout"
                                ? "center 18%"
                                : "center center",
                            transform:
                              config.photoRenderMode === "cutout"
