...
export async function GET(req: NextRequest) {
  try {
    ...
    const manualCategoryIds = (await listManualEventCategories(region.id)).map(
      (item) => item.id,
    );
    const permanentCategoryIds = (
      await listActivePermanentCategories(region.id)
    ).map((item) => item.id);
    const scopedCreatorProfiles = scopedCreatorDocs
      .filter((doc) => {
    ...
    const creatorIds = new Set(
      scopedCreatorProfiles.map((item) => item.creatorPublicId),
    );
    const scopedPosters = snapshot.posters.filter(
      (item) =>
        creatorIds.has(item.creatorPublicId) && item.regionId === region.id,
    );
    ...
    const pendingReviews = scopedPosters.filter(
      (item) => item.status === "pending",
    ).length;
    const approvedCount = scopedPosters.filter((item) =>
      isApprovedEquivalentStatus(item.status),
    ).length;
    const rejectedCount = scopedPosters.filter(
    ...
    return NextResponse.json({
      ...
      overview: {
        ...
        activeCreators: scopedCreatorProfiles.filter(
          (item) => item.status === "active",
        ).length,
        blockedCreators: scopedCreatorProfiles.filter(
          (item) => item.status === "blocked",
        ).length,
        pendingInvites: scopedCreatorProfiles.filter(
      ...
      performanceSummary: {
        ...
        approvalRate:
          totalUploads > 0
            ? Number(((approvedCount / totalUploads) * 100).toFixed(1))
            : 0,
      },
      uploadsTrend: buildUploadsTrend(
        scopedPosters.map((item) => item.createdAt),
      ),
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unable to load manager overview.";
