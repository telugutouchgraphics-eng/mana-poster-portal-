...
export default function AdminCreatorBannersPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      resetForm();
      setMessage(editingId ? "Creator banner updated." : "Creator banner saved.");
      await load();
    } catch (error) {
      ...
      <article className="rounded-[28px] border border-[var(--portal-border)] bg-white p-6 shadow-[0_12px_30px_rgba(15,23,42,0.05)]">
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          ...
          <div className="rounded-2xl border border-[var(--portal-border)] bg-[var(--portal-surface-soft)] p-4">
            ...
            <div className="overflow-hidden rounded-2xl border border-[var(--portal-border)] bg-white" style={{ aspectRatio: "1920 / 540" }}>
              {previewImage ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={previewImage} alt="Creator banner preview" className="h-full w-full object-cover" />
              ) : (
          ...
          <div className="flex flex-wrap gap-3">
            <button disabled={busy} type="submit" className="rounded-2xl bg-[var(--portal-purple)] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[var(--portal-purple-dark)] disabled:opacity-60">
              {busy ? "Saving..." : editingId ? "Update Creator Banner" : "Upload Creator Banner"}
            </button>
            {editingId ? (
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-xs text-slate-500">Placement: Creator Overview Banner | Position: {bannerPositionLabel(item.sortOrder)}</p>
                    </div>
